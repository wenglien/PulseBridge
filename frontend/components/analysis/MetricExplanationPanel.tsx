...
export function MetricExplanationPanel({ items, references }: MetricExplanationPanelProps) {
  ...
  if (!items.length) {
    return (
      <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-6 shadow-sm">
        <p className="text-xs text-[var(--text-3)] uppercase tracking-wider mb-3">指標解讀與改善</p>
        <p className="text-sm text-[var(--text-2)]">本次分析尚未產生逐項指標說明。</p>
      </div>
    )
  }
  ...
  return (
    <div className="bg-[var(--surface)] border border-[var(--border)] rounded-lg p-6 shadow-sm space-y-3">
      <div className="flex items-center justify-between flex-wrap gap-2">
        <p className="text-xs text-[var(--text-3)] uppercase tracking-wider">指標解讀與改善</p>
          ...
          return (
            <div
              ...
              className={cn(
                ...
                isExpanded ? "border-[var(--border-mid)] bg-[var(--surface-muted)]" : "border-[var(--border)] bg-[var(--surface-muted)]",
              )}
            >
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    <p className="text-sm font-semibold text-[var(--text-1)]">{item.metric_label_zh}</p>
                    <span className={cn("text-[10px] px-1.5 py-0.5 rounded border font-bold", priorityStyle(item.priority))}>
                      {priorityLabel(item.priority)}
                    </span>
                  </div>
                  {!isExpanded && (
                    <p className="text-xs text-[var(--text-2)] mt-0.5 truncate">{item.interpretation_zh}</p>
                  )}
                </div>
                <div className="flex items-center gap-2 shrink-0">
                  <span className="text-xs font-mono px-2 py-0.5 rounded-md bg-[var(--surface)] border border-[var(--border)] text-[var(--text-2)]">
                    {item.current_value || "—"}
                  </span>
                  <span className="text-xs text-[var(--text-3)] select-none">{isExpanded ? "收起" : "展開"}</span>
                </div>
              </button>
                ...
                <div className="px-4 pb-4 space-y-4 border-t border-[var(--border)] pt-3">
                  ...
                  <div className="space-y-3">
                    {[
                      ...
                      <div key={label}>
                        <p className="text-xs font-semibold text-[var(--text-3)] mb-1">{label}</p>
                    ...
                    {item.actionable_steps.length > 0 && (
                      <div>
                        <p className="text-xs font-semibold text-[var(--text-3)] mb-2">可採取行動</p>
                        <div className="flex flex-wrap gap-2">
                          {item.actionable_steps.map((step) => (
                            <span
                              key={step}
                              className="text-xs px-2.5 py-1 rounded-lg bg-[var(--surface)] border border-[var(--border)] text-[var(--text-2)]"
                            >
                              {step}
                            </span>
                          ))}
